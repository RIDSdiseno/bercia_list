...
async function main() {
  ...
  try {
    const v1 = await graphGet<{ value: any[] }>(
      `/sites/${cfg.siteId}/lists?includeHiddenLists=true&$select=id,displayName`
    );
  ...
  const { data } = await axios.get(
    `https://graph.microsoft.com/beta/sites/${cfg.siteId}/lists?includeHiddenLists=true&$select=id,displayName`,
    { headers: { Authorization: `Bearer ${token}` } }
  );

  console.log("=== LISTAS (beta includeHiddenLists) ===");
  data.value.forEach((l: any) => console.log(l.displayName, "->", l.id));
}
